...
output_file_path = os.path.join(FULL_SRC_IP_DIR_CONFIG, config_architecture_path, overlay_template_filename)
...
def get_engine_id(engine_name):
    ...
    engine_template_filename = f"{base_name}.ol" 
    ...
    append_to_file(output_file_path, f"// Name {base_name:<20}ID {engine_index:<4} mapping {base_mapping:<4} cycles {base_cycles:<4}")
    append_to_file(output_file_path, "// --------------------------------------------------------------------------------------")
    process_file(template_file_path, engine_template_filename)
    append_to_file(output_file_path, "// --------------------------------------------------------------------------------------")
    return base_mapping
...
def process_file(template_file_path, engine_template_filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if hex_match:
                ...
                append_to_file(output_file_path, f"{hex_value:<10} // entry {entry_index:<4} cacheline {cache_line:<4} offset {offset:<4} -- {comment}")
                entry_index += 1
