...
def main(params_sh_dir):
    ...
    cfg_file_name_sv = os.path.join(
        app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.sv.f"
    )
    cfg_file_name_v = os.path.join(
        app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.v.f"
    )
    cfg_file_name_vh = os.path.join(
        app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.vh.f"
    )
    cfg_file_name_vhdl = os.path.join(
        app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.vhdl.f"
    )
    ...
    configs = {
        ...
        os.path.join(app_dir_active, ip_dir_rtl_active, kernel) + "/": ["sv"],
        os.path.join(app_dir_active, ip_dir_rtl_active, top) + "/": ["v"],
        os.path.join(app_dir_active, ip_dir_rtl_active, testbench, testbench_module)
        + "/": ["sv"],
    }
    ...
    for path, extensions in configs.items():
        for extension in extensions:
            ...
            if extension == "sv":
                ...
                extension == "vhdl"
            ):  # Assuming svh files go into vhdl list for this example
                cfg_file = cfg_file_name_vhdl
            else:
                continue  # Skip if extension is not recognized
            generate_package_filelist(path, cfg_file, extension)
