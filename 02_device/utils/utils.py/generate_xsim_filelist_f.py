...
def generate_package_filelist(ip_directory, cfg_filelist_name, verilog_type):
    ...
    with open(cfg_filelist_name, 'a') as cfg_file:
        ...
        for filepath in sorted(glob.glob(os.path.join(ip_directory, pattern), recursive=True)):
...
def main(params_sh_dir):
    ...
    testbench_module = params['TESTBENCH_MODULE']
    ...
    cfg_file_name_sv = os.path.join(app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.sv.f")
    cfg_file_name_v = os.path.join(app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.v.f")
    cfg_file_name_vh = os.path.join(app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.vh.f")
    cfg_file_name_vhdl = os.path.join(app_dir_active, utils_dir_active, f"{kernel_name}_filelist_xsim.vhdl.f")
    
    # Clear file lists
    open(cfg_file_name_sv, 'w').close()
    open(cfg_file_name_v, 'w').close()
    ...
    topology_file_path = os.path.join(app_dir_active, utils_dir_active, topology_file_name)
    ...
    default_engines = ["engine_template", "engine_m_axi", "engine_cu_setup", "engine_pipeline"]
    ...
    for engine_dir in engine_dirs:
        ...
        generate_package_filelist(path, cfg_file_name_sv, "sv")
        generate_package_filelist(path, cfg_file_name_v, "v")
        generate_package_filelist(path, cfg_file_name_vh, "vh")
        generate_package_filelist(path, cfg_file_name_vhdl, "vhdl")
    
    # Remove package file list if empty
    remove_empty_file_lists(cfg_file_name_sv)
    remove_empty_file_lists(cfg_file_name_v)
    remove_empty_file_lists(cfg_file_name_vh)
    remove_empty_file_lists(cfg_file_name_vhdl)
