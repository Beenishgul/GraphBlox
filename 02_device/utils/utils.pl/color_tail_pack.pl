...
sub sub_print_close {
    ...
    print "\n", "=========================================================",
        "\n";
    print "[", BLUE, "Running Kernel Package", RESET, "]";
...
print "\n", "=========================================================", "\n";
print "[", BLUE, "Running Package Kernel", RESET, "]";
...
while (<STDIN>) {
   ...
   # print "=========================================================", "\n";
    ...
    if (@fields_s) {
        if ( $fields_s[0] ne "" ) {
            if    ( $fields_s[0] =~ /^\*/ ) { }
            elsif ( $fields_s[0] =~ /^INFO:/ ) {
                if ( $time_flag == 1 ) { sub_print_info($line); }
            }
            elsif ( $fields_s[0] =~ /^ERROR:/ ) { sub_print_error($line); }
            elsif ( $fields_s[0] =~ /^WARNING:/ ) {
                sub_print_warning($line);
            }
            ...
            elsif ( $fields_s[0] =~ /^$step_17$/ ) { sub_print_msg_V($line); }
            ...
            elsif ( $fields_s[0] =~ /^$step_9$/ ) {
                sub_print_section($line);
            }
            elsif ( $fields_s[0] =~ /^$step_10$/ ) { sub_print_step($line); }
            elsif ( $fields_s[0] =~ /^$step_11$/ ) {
                sub_print_finished($line);
            }
            elsif ( $fields_s[0] =~ /^$step_13$/ ) {
                sub_print_compiling($line);
            }

            elsif ( $line =~ /$step_12/ ) { sub_print_time($line); }
            elsif ( $fields_s[0] =~ /^\[[0-9][0-9]:[0-9][0-9]:[0-9][0-9]\]$/ )
            {
                sub_print_vpl($line);
            }
