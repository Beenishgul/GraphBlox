...
python3 ${APP_DIR_ACTIVE}/${UTILS_DIR_ACTIVE}/${UTILS_PYTHON}/generate_rtl_synth_cfg.py ${APP_DIR_ACTIVE} ${UTILS_DIR_ACTIVE} ${KERNEL_NAME} ${XILINX_IMPL_STRATEGY} ${XILINX_JOBS_STRATEGY} ${PART} ${PLATFORM} ${TARGET} ${XILINX_NUM_KERNELS} ${XILINX_MAX_THREADS} ${DESIGN_FREQ_HZ} ${FULL_SRC_IP_DIR_OVERLAY} ${ARCHITECTURE} ${CAPABILITY}
