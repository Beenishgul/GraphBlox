...
// contains (WIDTH_INT - 1) nodes, so resource usage of the
// arbiter grows linearly. The number of levels and thus the
// propogation delay down the tree grows with log2(WIDTH_INT).
...
module arbiter_bus_N_in_1_out #(
    parameter WIDTH        = 2                ,
    parameter WIDTH_INT    = 2**$clog2(WIDTH) ,
    parameter SELECT_WIDTH = $clog2(WIDTH_INT),
    parameter BUS_WIDTH    = 8
) (
    input  logic                 ap_clk                        ,
    input  logic                 areset                        ,
    input  logic                 arbiter_enable                ,
    input  logic [WIDTH_INT-1:0] arbiter_req                   ,
    input  logic [WIDTH_INT-1:0] arbiter_bus_valid             ,
    input  logic [BUS_WIDTH-1:0] arbiter_bus_in [WIDTH_INT-1:0],
    output logic [WIDTH_INT-1:0] arbiter_grant                 ,
    ...
    logic   [   WIDTH_INT-1:0] arbiter_grant_reg                   ;
    logic   [   WIDTH_INT-1:0] arbiter_bus_valid_reg               ;
    logic   [   BUS_WIDTH-1:0] arbiter_bus_reg      [WIDTH_INT-1:0];
    integer                    i                                   ;
