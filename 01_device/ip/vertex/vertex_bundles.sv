...
// --------------------------------------------------------------------------------------
    logic                done_int_reg ;
    vertex_bundles_state current_state;
    vertex_bundles_state next_state   ;
...
// --------------------------------------------------------------------------------------
    always_ff @(posedge ap_clk) begin
        if(areset_vertex_bundles)
            current_state <= VERTEX_BUNDLES_RESET;
        else begin
    ...
    always_comb begin
        ...
        case (current_state)
            VERTEX_BUNDLES_RESET : begin
                next_state = VERTEX_BUNDLES_IDLE;
            end
            VERTEX_BUNDLES_IDLE : begin
                if(descriptor_in_reg.valid & engine_serial_read_done_out & engine_serial_read_ready_out)
                    next_state = VERTEX_BUNDLES_REQ_START;
                else
                    next_state = VERTEX_BUNDLES_IDLE;
            end
            VERTEX_BUNDLES_REQ_START : begin
                if(engine_serial_read_done_out & engine_serial_read_ready_out) begin
                    next_state = VERTEX_BUNDLES_REQ_START;
                end else begin
                    next_state = VERTEX_BUNDLES_REQ_BUSY;
                end
            end
            VERTEX_BUNDLES_REQ_BUSY : begin
                if (done_int_reg)
                    next_state = VERTEX_BUNDLES_REQ_DONE;
                else if (fifo_request_out_signals_out_int.prog_full | fifo_response_in_signals_out_int.prog_full)
                    next_state = VERTEX_BUNDLES_REQ_PAUSE;
                else
                    next_state = VERTEX_BUNDLES_REQ_BUSY;
            end
            VERTEX_BUNDLES_REQ_PAUSE : begin
                if (~(fifo_request_out_signals_out_int.prog_full | fifo_response_in_signals_out_int.prog_full))
                    next_state = VERTEX_BUNDLES_REQ_BUSY;
                else
                    next_state = VERTEX_BUNDLES_REQ_PAUSE;
            end
            VERTEX_BUNDLES_REQ_DONE : begin
