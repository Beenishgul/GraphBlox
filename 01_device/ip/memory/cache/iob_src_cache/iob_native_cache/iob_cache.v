...
module iob_cache #(
  ...
  `ifdef WORD_ADDR
  input  [(CACHE_CTRL_CACHE+CACHE_FRONTEND_ADDR_W-CACHE_FRONTEND_BYTE_W)-1:0] addr         , //Native CPU interface address signal
  `else
  input  [                      (CACHE_CTRL_CACHE+CACHE_FRONTEND_ADDR_W)-1:0] addr         , //Native CPU interface address signal
  `endif
  input  [                                       (CACHE_FRONTEND_DATA_W)-1:0] wdata        , //Native CPU interface data write signal
  input  [                                       (CACHE_FRONTEND_NBYTES)-1:0] wstrb        , //Native CPU interface write strobe signal
  output [                                       (CACHE_FRONTEND_DATA_W)-1:0] rdata        , //Native CPU interface read data signal
  output                                                                      ready        , //Native CPU interface ready signal
  `ifdef CTRL_IO
  //control-status io
  //START_IO_TABLE cs_io
  input                                                                       force_inv_in , //force 1'b0 if unused
  output                                                                      force_inv_out, //cache invalidate signal
  input                                                                       wtb_empty_in , //force 1'b1 if unused
  output                                                                      wtb_empty_out, //write-through buffer empty signal
  `endif
  //Slave i/f - Native
  //START_IO_TABLE iob_s
  output                                                                      mem_valid    , //Native CPU interface valid signal
  output [                                          CACHE_BACKEND_ADDR_W-1:0] mem_addr     , //Native CPU interface address signal
  output [                                          CACHE_BACKEND_DATA_W-1:0] mem_wdata    , //Native CPU interface data write signal
  output [                                          CACHE_BACKEND_NBYTES-1:0] mem_wstrb    , //Native CPU interface write strobe signal
  input  [                                          CACHE_BACKEND_DATA_W-1:0] mem_rdata    , //Native CPU interface read data signal
  input                                                                       mem_ready      //Native CPU interface ready signal
);
