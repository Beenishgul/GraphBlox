    ...
    always_comb begin
        ...
        case (current_state)
            ...
            ENGINE_SERIAL_READ_IDLE : begin
                if(configuration_reg.valid && start_reg)
...
// --------------------------------------------------------------------------------------
    ...
    always_ff @(posedge ap_clk) begin
        if(start_reg) begin
            fifo_request_din.payload.meta <= fifo_request_comb.payload.meta;
        end else begin
            fifo_request_din.payload.meta <= 0;
        end
    end
