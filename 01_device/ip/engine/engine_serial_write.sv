...
module engine_serial_write #(parameter COUNTER_WIDTH      = 32) (
    // System Signals
    input  logic                         ap_clk                  ,
    input  logic                         areset                  ,
    input  SerialWriteEngineConfiguration configuration_in        ,
    output MemoryPacket                  request_in              ,
...
// --------------------------------------------------------------------------------------
    ...
    SerialWriteEngineConfiguration configuration_reg;
    MemoryPacket                  request_out_reg  ;

// --------------------------------------------------------------------------------------
//   Setup state machine signals
// --------------------------------------------------------------------------------------
    engine_serial_write_state current_state;
    engine_serial_write_state next_state   ;
    ...
    always_ff @(posedge ap_clk) begin
        case (current_state)
            ...
            ENGINE_SERIAL_READ_SETUP : begin
                ...
                counter_load_value         <= configuration_reg.payload.param.start_write;
            ...
            ENGINE_SERIAL_READ_BUSY : begin
                if((counter_count >= configuration_reg.payload.param.end_write)) begin
                    done_internal_reg          <= 1'b1;
                    counter_incr               <= 1'b0;
                    counter_decr               <= 1'b0;
                    fifo_request_din_reg.valid <= 1'b0;
