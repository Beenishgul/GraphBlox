                           ...
                           struct Arguments *arguments, struct Timer *timer)
{
    ...
    for (uint32_t i = 0; i < graph->num_vertices; i++)
    {
        ...
        if(static_cast<uint32_t *>(graphAuxiliary->auxiliary_2)[i + graph->num_vertices] != vertices->out_degree[i])
        {
            DEBUG_PRINT("%u %u \n",
                        static_cast<uint32_t *>(
                            graphAuxiliary->auxiliary_2)[i + graph->num_vertices], vertices->out_degree[i]);
        }
